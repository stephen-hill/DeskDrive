...
namespace BlueOnion
{
    ...
    {
        private static IntPtr ShellViewWindow = IntPtr.Zero;

        static IntPtr GetDesktopHandle()
        {
        ...
        static IntPtr GetDesktopProcess(IntPtr desktopHandle)
        {
            uint vProcessId;
            ...
            for (var item = 0; item < vItemCount; item++)
            {
                ...
                if (string.Compare(title, 0, iconTitle, 0, title.Length, StringComparison.CurrentCultureIgnoreCase) == 0)
                {
                    ...
                    try
                    {
                        ...
                        vPoint[0].X += Properties.Settings.Default.MonitorOffset.Width;
                        vPoint[0].Y += Properties.Settings.Default.MonitorOffset.Height;
                        return vPoint[0];
                    }
        ...
        const string IconPositionsFile = "IconPositions.txt";
        static Dictionary<string, Point> _desktopIconPositions;
